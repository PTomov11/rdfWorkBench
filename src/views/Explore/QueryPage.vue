<template>
  ...
  <div class="main">
    ...
    <DataTable v-if="executed" :value="queryResults" :paginator="true" :alwaysShowPaginator="false" :rows="9" :loading="loading"
               paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
               :rowsPerPageOptions="[10,20,50]" responsiveLayout="scroll"
               currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
      <Column v-for="(col,index) of columns"  :header="col.header" :key="col.field" >
        <template #body="slotProps">
          <span v-if="slotProps.data[index].length > 30" v-tooltip.right="{ value: slotProps.data[index] }">{{truncate(slotProps.data[index], 30, '...')}}</span>
      ...
      <DataTable :value="savedQueries" responsiveLayout="scroll">
        ...
        <Column :exportable="false" style="min-width:8rem">
          <template #body="slotProps">
            ...
            <Button icon="pi pi-user-edit" class="p-button-rounded" style="margin-left: 20px;" @click="editQuery(slotProps.data)" />
            <Button icon="pi pi-trash" class="p-button-rounded p-button-danger" style="margin-left: 20px;" @click="confirmDeleteQuery(slotProps.data)" />
          </template>
        </Column>
      </DataTable>
    </div>
